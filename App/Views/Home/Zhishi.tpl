...
<form class="zhishi form-horizontal" action="?z=home/zhishi"  method="post">
	{foreach from=$jiab key=k item=v}
		<h2>嘉宾 : {$v['jiab']} <small>演讲主题 :{$v['jiatitle']}</small></h2>
		...
		{foreach from=$res key=key item=value name=foo}
			{if $v['jiab'] eq $value['jiab']}
			...
			<div class="result-box clearfix">
				<div class="fl">
					...
					{foreach from=$res item=value name=foo}
						{math equation=$i+1 assign=i}
						{if ($i-1) eq 5 ||  ($i-1) eq 10 ||  ($i-1) eq 15 ||  ($i-1) eq 20 ||  ($i-1) eq 25 ||  ($i-1) eq 30 ||  ($i-1) eq 35 ||  ($i-1) eq 40 ||  ($i-1) eq 45 ||  ($i-1) eq 50}<br>{/if}
					{$smarty.foreach.foo.index+1}:<strong class="red">{$value['da']}</strong>
                 {/foreach}
				</div>
			</div>
            {/if}
